...
const createExtraChargeTable = async (
    ...
    const extraChargeDataArray = await CasaDB.ExtraCharges.findAll({
        attributes: ["id", "title"],
        include: [
            {
                attributes: ["id", "companyName"],
                model: CasaDB.Company,
            },
        ],
        where: {
            isActive: 1,
...
const newPdfGenration = async (
    ...
    try {
        ...
        if (
            ...
            const plantCodeId = data.reviseQuoteData.quoteObject.plantId
            let plantFolder
            ...
            let ss: Array<any> = data.reviseQuoteData.QualityControlSubmitted
                ? data.reviseQuoteData.QualityControlSubmitted
                : []
...
const HTMLReviseGeneration = async (data: any) => {
    try {
        const plantId = data.reviseQuoteData.quoteObject.plantId // Get Plant Id
        ...
        //! Quote Details
        const quoteData = data.reviseQuoteData.quoteObject
        //! Extra charge Section
        const extraChargeData = data.reviseQuoteData.extracharge
        //! Get Over Time Details
        const overTimeData = data.reviseQuoteData.overTime
        //! Get primumRate Deatils
        const primumRateData = data.reviseQuoteData.primumRateCharge
        //! Short Load Charges Details
        const shortChargeData = data.reviseQuoteData.shortCharge
        //! Mix Design Data
        const productRowData = data.reviseQuoteData.prouctData
            .map(productRow)
            .join("")
        ...
        const termsRowData = data.reviseQuoteData.termsConditionTableData
        //! terms Condition Details
        const termsDetailsRowData = data.reviseQuoteData.termsConditionDetailsData
        ...
        //! Quote File Name
        const quoteFileName = data.reviseQuoteData.quoteFileName
        const quoteId = data.reviseQuoteData.quoteObject.quoteId
        ...
        const orignalFilePath = `public/${plantFolder}/Reviseddocument/${quoteFileName}`
        ...
        const userId = data.reviseQuoteData.quoteObject.userId
